-- Carb Quiz · Supabase schema
-- Run in: Supabase Dashboard → SQL Editor → New query → Run
-- Project: ungdcpmarivrrhhmvdbb

-- ── Quiz catalog (supports multiple quizzes later) ──────────────────────────
...
create index if not exists quiz_attempts_student_name_idx
  on quiz_attempts (student_name);

create index if not exists quiz_attempts_score_idx
  on quiz_attempts (score desc);

-- ── Per-question answers (normalized for analytics at scale) ────────────────
...
create index if not exists quiz_answer_details_attempt_idx
  on quiz_answer_details (attempt_id);

create index if not exists quiz_answer_details_question_idx
  on quiz_answer_details (question_num);

-- ── Analytics view: % correct per question ──────────────────────────────────
